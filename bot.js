...
<blockquote>⚠️ Saldo tidak bisa ditarik ke akun e-wallet.</blockquote>`,
    ...
    const first = msg.from.first_name || "Pengguna";
    const myRef = `REF${chatId}`;
    const caption = `
👋 𝗛𝗮𝗹𝗼, <b>${first}</b>!
Selamat datang di <b>ＫＡＩＩ ＭＡＲ𝗞ＥＴ</b> ⚡

🤖 Bot ini siap bantu kamu bikin <b>panel otomatis</b> — cepat, stabil, dan tanpa ribet!
Semua proses tinggal klik, dan sistem bakal langsung jalan sendiri 🚀

💎 Dapatkan <b>saldo tambahan Rp2000</b> setiap kali kamu mengundang teman!
Gunakan link referral pribadimu:
...
🎁 Mau panel gratis?
Ketik:
<code>/gratis kaii</code> — untuk klaim panel gratis 😋😋😋

────────────────────────────
🔽 Pilih menu di bawah untuk mulai eksplor fitur keren dari <b>KaiiMarket</b>.
💎 Siap ngerasain pengalaman panel yang beda dari yang lain?
`;
...
⚠️ Pastikan saldo kamu cukup sebelum membuat panel.`;
    ...
    // 💰 DEPOSIT MENU
    if (dataQ === "menu_deposit") {
        const text = `
💰 <b>Deposit Saldo</b>
────────────────────────────
📲 Scan QRIS di bawah untuk melakukan deposit saldo.

Kirim bukti pembayaran dengan format:
Gambar dengan text
<code>/bayar </code>
...
/bayar  – 🧾 Kirim bukti ke admin
/stalk id_user – 👀 Info & foto profil user
/stat – 📊 Statistik bot (owner)
...
/listdiskon - 🤔 Cek harga panel sedang diskon atau tidak`;
    ...
    // 🏠 KEMBALI
    if (dataQ === "menu_home") {
        const first = query.from.first_name || "Pengguna";
...
<blockquote>🎁 Mau panel gratis?
Ketik:
<code>/gratis kaii</code> — untuk klaim panel gratis 😋😋😋</blockquote>
...
🕓 Admin akan memverifikasi secara manual.`;
     ...
     bot.sendPhoto(msg.chat.id, settings.qr, { caption, parse_mode: "HTML" });
});

// ==========================================================
// 🧾 BAYAR
// ==========================================================
// Konversi string waktu (misal "2025-10-20 12:56") ke Date dengan zona WIB
function parseToDate(str) {
    if (!str) return null;

    // Pastikan string waktu diubah ke format ISO dan ditambahkan offset WIB
    // Misal: "2025-10-20 12:56" → "2025-10-20T12:56:00+07:00"
    const normalized = str
        .replace(" ", "T")
        .replace(/\//g, "-")
        .replace(/(\d{2}):(\d{2})(?!:)/, "$1:$2:00") + "+07:00";
...
// Hitung selisih jam antara dua Date (hasil selalu positif)
function jamSelisih(d1, d2) {
...
// hitung persentase kemiripan string sederhana
...
bot.onText(/^\/bayar$/, async msg => {
    const chatId = msg.chat.id;
     ...
     bot.sendPhoto(chatId, "https://files.catbox.moe/sgmxkh.jpg", {
        caption:
            "📸 Kirim gambar bukti pembayaran(screenshot) seperti gambar diatas."
    });
});

// handler gambar hanya jika caption /bayar
bot.on("photo", async msg => {
    const chatId = msg.chat.id;
    const caption = (msg.caption || "").trim().toLowerCase();

    // kalau tidak ada caption /bayar → abaikan
    if (caption !== "/bayar") return;

    const fileId = msg.photo[msg.photo.length - 1].file_id;
    const fileLink = await bot.getFileLink(fileId);

    bot.sendMessage(chatId, "🔍 Sedang menganalisis gambar dengan AI...");

    try {
        // kirim ke OpenAI Vision
                    ...
                    content: [
                        {
                            type: "input_text",
                            text: `
Kamu adalah sistem verifikasi bukti pembayaran otomatis.

Analisis gambar struk pembayaran digital (DANA, GoPay, OVO, ShopeePay, LinkAja, Bank, dsb).  
Ekstrak semua informasi penting meskipun teksnya tidak eksplisit.

Fokus utama:
- Deteksi waktu transaksi meski hanya tertulis seperti "20 Okt 2025 · 16:47" atau format serupa tanpa kata "Waktu".
- Deteksi nominal sekecil apa pun seperti "Rp1", "Rp2", dst.
- Deteksi status meski tidak eksplisit (misal “Transaksi berhasil” → “Berhasil”).
- Jika provider tidak jelas, tebak berdasarkan logo, warna, atau teks seperti “DANA”, “GoPay”, “ShopeePay”.
- Jika tidak ada waktu yang jelas, ambil teks tanggal/waktu yang paling mirip dengan waktu transaksi.
- Jika tidak yakin, isi nilai yang paling mendekati kemungkinan sebenarnya.
- Deteksi juga penerima pembayaran (contoh: "kaiimarket", "tokopedia", dll) bila ada.

Kembalikan hanya JSON valid dengan struktur:
{
  "valid": true/false,
  "provider": "DANA/GoPay/OVO/ShopeePay/Bank/Unknown",
  "nominal": number,
  "status": "Berhasil/Gagal/TidakDiketahui",
  "waktu_transaksi": "YYYY-MM-DD HH:mm (jika terdeteksi, jika tidak isi null)",
  "penerima": "string atau null"
}
`
                        },
                        { type: "input_image", image_url: fileLink }
                    ]
                }
            ]
        });
        ...
        // setelah parsed = JSON.parse(textOutput);
        if (!parsed.valid || parsed.status?.toLowerCase() !== "berhasil") {
         bot.sendPhoto(chatId, contohBuktiUrl, {
                caption:
                    "❌ Bukti pembayaran tidak valid atau gagal dibaca.\n\n" +
                    "Harap kirim bukti transfer seperti contoh di atas."
            });
            return;
        }

        // cek waktu transaksi
        if (!parsed.waktu_transaksi) {
             bot.sendPhoto(chatId, contohBuktiUrl, {
                caption:
                    "❌ Waktu transaksi tidak jelas.\n\n" +
                    "Harap kirim bukti transfer seperti contoh di atas."
            });
            return;
        }
        ...
        // cek selisih waktu
const nominal = parsed.nominal || 0;
const waktuUser = nowWIB();
const waktuTransaksi = parseToDate(parsed.waktu_transaksi);
const selisihMenit = Math.abs((waktuUser - waktuTransaksi) / (1000 * 60));
if (selisihMenit > 10) {
    bot.sendMessage(
        chatId,
        "❌ Waktu transaksi berbeda lebih dari 10 menit dari waktu pengiriman."
    );
    return;
}

        // cek penerima
        const penerima = parsed.penerima || "";
        if (!penerima || similarity(penerima, "KaiiMarket") < 60) {
             bot.sendPhoto(chatId, contohBuktiUrl, {
                caption:
                    `❌ Bukti pembayaran tampaknya tidak ditujukan ke 'kaiimarket'.\n\n` +
                    "Harap kirim bukti transfer seperti contoh di atas."
            });
            return;
        }

        // jika valid, lanjut saja tanpa else
        ...
        saveData(); // sudah ada di kode kamu
         ...
         bot.sendMessage(
            ...
            `✅ Bukti pembayaran valid!\n` +
                ...
                `💰 Nominal: Rp${nominal}\n` +
                `Saldo kamu sekarang: Rp${data[chatId].saldo}`
        );

        // kirim log ke admin
         bot.sendMessage(
            OWNER_ID,
            `📥 Pembayaran baru dari ${msg.from.first_name} (${chatId})\n` +
                `Provider: ${parsed.provider}\nNominal: Rp${nominal}`
        );
    } catch (err) {
        console.error("Error:", err);
         bot.sendMessage(chatId, "❌ Gagal memproses gambar dengan AI.");
    }
});
...
bot.onText(/\/cek/, msg => {
    const chatId = msg.chat.id;
    const saldo = data[chatId]?.saldo || 0;
...
💰 Saldo Saat Ini: <code>Rp${saldo.toLocaleString()}</code>
────────────────────────────
🧭 Gunakan saldo kamu untuk membeli panel:
<code>/1gb NamaKamu</code> atau <code>/unli NamaKamu</code>

<blockquote>ℹ️ Catatan: saldo hanya dapat digunakan di dalam sistem dan tidak dapat ditarik ke e-wallet.</blockquote>
`;
...
bot.onText(/\/stalk (\S+)/, async (msg, match) => {
    ...
    try {
        ...
        // 🚫 Cegah stalk grup atau channel
        if (info.type !== "private") {
            return bot.sendMessage(
                chatId,
                "⚠️ Fitur /stalk hanya untuk <b>user pribadi</b>, bukan grup atau channel.",
                { parse_mode: "HTML" }
            );
        }

        // Format info user
        let teks = `👤 <b>INFORMASI USER</b>\n━━━━━━━━━━━━━━\n`;
        ...
        // === Kirim foto profil kalau ada, langsung dari buffer ===
        if (info.photo) {
...
bot.onText(/\/broadcast/, async msg => {
    ...
    // ✅ Hanya OWNER yang bisa kirim broadcast
    if (chatId !== OWNER_ID)
        return bot.sendMessage(chatId, "🚫 Akses ditolak!");

    // ⚠️ Pastikan owner reply ke pesan yang ingin dikirim
    if (!msg.reply_to_message)
        return bot.sendMessage(
            chatId,
            "⚠️ Balas pesan yang ingin di-broadcast!\n\nContoh:\nReply ke pesan lalu kirim /broadcast"
        );
    ...
    bot.sendMessage(chatId, "🚀 Memulai broadcast ke semua user...");

    for (const id of Object.keys(data)) {
        try {
            // 1️⃣ Kirim header dulu
            await bot.sendMessage(id, "📢 <b>PENGUMUMAN:</b>", { parse_mode: "HTML" });

            // 2️⃣ Tunggu sedikit supaya header tampil duluan
            await new Promise(res => setTimeout(res, 300));

            // 3️⃣ Baru kirim isi pesannya
            await bot.forwardMessage(id, chatId, targetMessage.message_id);

            sukses++;
            await new Promise(res => setTimeout(res, 100)); // jeda antar user
        } catch (err) {
            gagal++;

            // Cek apakah user blokir bot / user tidak ditemukan
            const msgError = err.response?.body?.description || err.message || "";

            if (
                msgError.includes("bot was blocked by the user") ||
                msgError.includes("user not found") ||
                msgError.includes("chat not found") ||
                msgError.includes("Forbidden")
            ) {
                console.log(`⚠️ User ${id} gagal dikirimi pesan (${msgError}). Menghapus user...`);
                delete data[id];
                dihapus++;
                saveData();
            } else {
                console.log(`❌ Gagal kirim ke ${id}: ${msgError}`);
            }
        }
    }

    // 4️⃣ Kirim laporan ke owner
    bot.sendMessage(
        OWNER_ID,
        `✅ Broadcast selesai!\n\n🟢 Sukses: ${sukses}\n🔴 Gagal: ${gagal}\n🗑️ Dihapus: ${dihapus}\n👥 Total user aktif: ${Object.keys(data).length}`
    );

    console.log(`✅ Broadcast selesai — ${sukses} sukses, ${gagal} gagal, ${dihapus} user dihapus.`);
});
...
bot.onText(/\/setapikey(?:\s+(.+))?/, async (msg, match) => {
    ...
    if (!args || args.length < 3) {
        return bot.sendMessage(
            chatId,
            "❌ Format salah.\nGunakan:\n`/setapikey (domain) (plta) (pltc)`\n\nContoh:\n`/setapikey 0 ptla_BARU123 0`",
            { parse_mode: "Markdown" }
        );
    }

    const [domainArg, pltaArg, pltcArg] = args;

    // Ganti hanya yang tidak bernilai "0"
    const newData = {
        domain: domainArg !== "0" ? domainArg : panelData.domain,
        plta: pltaArg !== "0" ? pltaArg : panelData.plta,
        pltc: pltcArg !== "0" ? pltcArg : panelData.pltc
    };

    // Simpan ke panel.json
    fs.writeFileSync("./panel.json", JSON.stringify(newData, null, 2));

    // Perbarui data runtime tanpa restart
    panelData = loadPanelData();
    settings.domain = panelData.domain;
    settings.plta = panelData.plta;
    settings.pltc = panelData.pltc;

    // Kirim konfirmasi
    const text =
        `✅ *panel.json & settings berhasil diperbarui!*\n` +
        `🌐 Domain: ${settings.domain}\n` +
        `🔑 PLTA: ${settings.plta}\n` +
        `🔑 PLTC: ${settings.pltc}`;
    bot.sendMessage(chatId, text, { parse_mode: "Markdown" });

    console.log("✅ settings diperbarui otomatis:", settings);
});
...
bot.onText(/\/lihatapikey/, (msg) => {
    ...
    // Pastikan data panel tersedia
    if (!panelData || !panelData.domain) {
        return bot.sendMessage(chatId, "❌ Data panel belum tersedia. Silakan atur dulu dengan /setapikey");
    }

    // Buat teks tampil rapi
    const text = `
📄 <b>Data panel.json saat ini:</b>

🌐 <b>Domain:</b> ${panelData.domain || "-"}
🔑 <b>PLTA:</b> <code>${panelData.plta || "-"}</code>
🔑 <b>PLTC:</b> <code>${panelData.pltc || "-"}</code>
    `.trim();
...
bot.onText(/\/cekapi/, async (msg) => {
    ...
    const panel = settings.domain;       // Contoh: https://panel.domainkamu.com
    const apiKey = settings.plta;        // Gunakan API Admin
    const nodes = ["plta", "pltc"];      // Daftar node yang ingin dicek

    let hasil = "<b>🔍 PTERODACTYL STATUS CHECK</b>\n────────────────────────────\n";

    try {
        // 1️⃣ Cek domain
        hasil += "🌐 <b>Panel:</b> " + panel + "\n";
        try {
            const res = await fetch(panel);
            hasil += res.ok ? "✅ Domain aktif\n" : "❌ Domain tidak bisa diakses\n";
        } catch {
            hasil += "❌ Gagal menghubungi domain panel\n";
        }

        // 2️⃣ Cek API
        try {
            const apiRes = await fetch(`${panel}/api/client`, {
                headers: { Authorization: `Bearer ${apiKey}` },
            });
            hasil += apiRes.ok ? "✅ API aktif\n" : "❌ API gagal / tidak valid\n";
        } catch {
            hasil += "❌ Gagal menghubungi API\n";
        }
        ...
        for (const node of nodes) {
            try {
                const res = await fetch(`${panel}/api/client/servers/${node}`, {
                    headers: { Authorization: `Bearer ${apiKey}` },
                });

                if (!res.ok) {
                    hasil += `❌ ${node.toUpperCase()} → Tidak bisa diakses\n`;
                    continue;
                }

                const data = await res.json();
                const status = data?.attributes ? "🟢 Online" : "🔴 Offline";
                hasil += `✅ ${node.toUpperCase()} → ${status}\n`;
            } catch {
                hasil += `❌ ${node.toUpperCase()} → Error saat cek\n`;
            }
        }
        ...
        console.error("[ERROR /cekapi]", err);
        bot.sendMessage(chatId, `❌ Terjadi kesalahan saat mengecek API.\n${err.message}`);
    }
});
...
setInterval(kirimPengingatKlaim, 60 * 60 * 1000);

// 🚀 Jalankan pertama kali setelah bot nyala, tapi tunggu 1 menit dulu
setTimeout(() => {
    console.log("⏳ Menunggu 1 menit sebelum kirim pengingat klaim pertama...");
    kirimPengingatKlaim();
}, 300 * 1000);
...
bot.onText(/\/addsaldo (\d+)\s+(\d+)/, (msg, match) => {
   ...
   bot.sendMessage(
        userId,
        `💰 Saldo kamu bertambah Rp${nominal.toLocaleString()} oleh admin!
💳 Saldo sekarang: Rp${data[userId].saldo.toLocaleString()}
...
bot.onText(/\/del (\d+)/, (msg, match) => {
    ...
    // Cek apakah user ada di data.json
    if (!data[userId]) {
        return bot.sendMessage(
            msg.chat.id,
    ...
    try {
        // Hapus user dari data lokal
        delete data[userId];
        saveData(); // Pastikan fungsi saveData() menyimpan data ke data.json

      bot.sendMessage(
            msg.chat.id,
            `✅ User dengan ID ${userId} berhasil dihapus dari data.json.`
        );
    } catch (err) {
...
<code>/chat ${fromId} pesan</code>`,
        ...
        bot.sendMessage(
            fromId,
            `❌ Gagal mengirim pesan ke ${targetId}\n${e.message}`
        );
    }
});
...
// ==========================================================
bot.onText(/\/buatadmin (.+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    ...
    try {
        ...
        // Buat user baru dengan hak admin
        const newUser = await fetch(`${settings.domain}/api/application/users`, {
            ...
            body: JSON.stringify({
                ...
                language: "en",
                password,
                root_admin: true // 🚨 Penting: jadikan admin panel
            })
        });
...
────────────────────────────
🚀 Sekarang akun ini bisa login sebagai <b>Admin</b>.
`, { parse_mode: "HTML" });
        ...
        try {
            ...
            for (const userId of userIds) {
                ...
                if (delUser.status === 204) {
                    ...
                    console.log(`✅ User ID ${userId} berhasil dihapus.`);
                } else {
                    const errText = await delUser.text();
                    bot.sendMessage(chatId, `❌ Gagal hapus user ID ${userId}.\nRespon: ${errText}`);
                    console.log(`❌ Gagal hapus user ID ${userId}.\n${errText}`);
                }
            }

            bot.sendMessage(chatId, `🎯 Proses selesai! Total user: ${userIds.length} telah diproses.`);
...
🔑 Password: <code>${password}</code>
🌐 Panel: ${settings.domain}
💰 Saldo: Rp${data[chatId].saldo.toLocaleString()}
────────────────────────────
🚀 Selamat! Server kamu siap digunakan.`,
            parse_mode: "HTML",
            reply_markup: {
                inline_keyboard: [
                    [
                        {
                            text: "Ya elah, terimakasih aja susah 🤭",
                            url: "https://t.me/ku_kaii?text=Makasih+bang+udah+ngasih+panel+gratis+🔥"
                        }
                    ],
                    [
                        {
                            text: "Kalo limit chat lewat sini 💬",
        ...
        bot.sendMessage(
            chatId,
            `<blockquote>Minimal terimakasih kek ke owner 🤭
Udah dikasih panel gratis masih aja diem, parah 😹</blockquote>`,
            { parse_mode: "HTML" }
        );
...
// ==========================================================
async function buatpanel(chatId, namaUser, paket, gratis = false) {
    ...
    try {
        ...
        // 🚫 Jika username/email sudah ada, hentikan proses
        if (existing) {
            return bot.sendMessage(
                chatId,
...
for (const paket in paketHarga) {
  bot.onText(new RegExp(`\\/${paket} (.+)`), async (msg, match) => {
    ...
    const targetName = msg.from.username ? `@${msg.from.username}` : msg.from.first_name;
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(nama)) {
      return bot.sendMessage(chatId, "⚠️ Nama hanya boleh huruf, angka, _ atau -");
    }

    // 🧩 CEK APAKAH PERINTAH DARI GRUP
    if (fromGroup) {
      // Coba kirim pesan ke private chat
      try {
        const testMsg = await bot.sendMessage(fromId, "🔍 Cek koneksi ke bot...");
        await bot.deleteMessage(fromId, testMsg.message_id);
        // ✅ Jika sukses kirim dan hapus → user sudah /start bot

      } catch (err) {
        console.error("[CEK PRIVATE CHAT ERROR]", err.message);
        // ❌ Jika gagal → user belum start bot
        await bot.sendMessage(
          chatId,
          `⚠️ Kamu belum /start bot di private chat!\nKlik link ini dulu: [Start @${botUsername}](https://t.me/${botUsername}?start=REF${OWNER_ID})`,
          { parse_mode: "Markdown" }
        );
    ...
    // === JIKA DARI GRUP RESMI ===
    if (fromGroup && msg.chat.id === GRUP_GRATIS) {
      await bot.sendMessage(
        chatId,
        `✨ *Membuat Panel Sedang Diproses...*\n\n` +
          `🆓 Karena kamu menggunakan perintah di grup resmi, panel ini akan dikirim langsung ke ${targetName}.\n\n` +
          `⚠️ *Pastikan kamu sudah /start @${botUsername} di chat pribadi terlebih dahulu!* ` +
          `Jika belum, bot tidak bisa mengirim detail panel ke kamu.\n\n` +
          `⏳ Mohon tunggu sebentar, panel sedang dibuat...`,
        { parse_mode: "HTML" }
      );

      await delay(500);
      await buatpanel(fromId, nama, paketNama, true);

      try {
        await delay(500);
        await bot.sendMessage(chatId, `✅ Panel untuk ${targetName} sukses dikirim ke private chat!`);
      } catch (e) {
        console.error("[ERROR kirim panel gratis ke chat privat]", e.message);
      }
    } else {
      // 💰 Jika bukan dari grup resmi
      await bot.sendMessage(chatId, "💳 Saldo kamu akan dipotong untuk membuat Panel.");
      await delay(700);
      await buatpanel(fromId, nama, paketNama, false);
    }
  });
}
...
setInterval(async () => {
    ...
    try {
        ...
        const dataFile = "./data.json"; // lokasi file data
